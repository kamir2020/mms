...
class SFormRiverManualImageRequest extends StatefulWidget {
  _SFormRiverManualImageRequest createState() => _SFormRiverManualImageRequest();
}

class _SFormRiverManualImageRequest extends State<SFormRiverManualImageRequest> {
  ...
  Future<void> _loadLocation(String? stateID) async {
    final data = await DBHelper.getLocationImageRiver(stateID);
    setState(() {
      _isLocation = true;
      location1 = data;
    });
  ...
  // ====== API ======
  Uri _buildUri() {
    const base = 'https://mmsv2.pstw.com.my/get-river-manual-image-request.php';
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // Location (station)
          _isLocation
              ? DropdownSearch<localRiverLocation>(
            items: location1,
            selectedItem: selectedLocalLocation,
            itemAsString: (localRiverLocation u) =>
            "${u.stationID} - ${u.riverName}",
            ...
            onChanged: (localRiverLocation? value) async {
              setState(() {
