...
class SFormRiverISDataLog extends StatefulWidget {
  _SFormRiverISDataLog createState() => _SFormRiverISDataLog();
}

class _SFormRiverISDataLog extends State<SFormRiverISDataLog> {
  ...
  Future<List<Map<String, dynamic>>> fetchDataRiverISSampleFromServer() async {

    final response = await http.get(Uri.parse('https://mmsv2.pstw.com.my/api/river/api-get-river.php?action=display-log-is-sample'));
  ...
  Future<List<Map<String, dynamic>>> fetchDataRiverISSampleFromLocal() async {
    final db = await DBHelper.initDb();
    final result = await db.rawQuery('''
    SELECT tbl_river_is_sampling.stationID,tbl_river_is_sampling.timestamp,
    tbl_river_is_sampling.stationName
  ...
  Widget buildFromServerRiverISSample() {
    return SizedBox(
      height: 500,
      child: FutureBuilder<List<Map<String, dynamic>>>(
        future: fetchDataRiverISSampleFromServer(),
  ...
  Widget buildFromLocalRiverISSample() {
    return SizedBox(
      height: 500,
      child: FutureBuilder<List<Map<String, dynamic>>>(
        future: fetchDataRiverISSampleFromLocal(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting)
            return Center(child: CircularProgressIndicator());
          if (snapshot.hasError)
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          Divider(),
          if (_selectedItem == 1) ... [
            connectionStatus ? buildFromServerRiverISSample() : buildFromLocalRiverISSample(),
          ]
