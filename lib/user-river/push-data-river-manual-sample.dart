...
class PushRiverManualSample {
  final int rowsPushed;
  final int imagesUploaded;
  final List<String> logs;

  const PushRiverManualSample({
...
class RiverManualSampleService {
  // Config
  final String dbName;
  ...
  RiverManualSampleService({
    ...
    Future<Directory> Function()? getDocsDir,
      ...
      Uri.parse('https://mmsv2.pstw.com.my/api/river/api-push-data.php'),
        uploadApiUrl = uploadApiUrl ??
            Uri.parse(
                'https://mmsv2.pstw.com.my/upload-image-local.php?image=river_manual_sample'),
        _openDb = openDb,
  ...
  Future<PushRiverManualSample> pushDataRiverManualSampleToMySQL({Logger? log}) async {
    final _logs = <String>[];
    ...
    try {
      ...
      await db.rawQuery('SELECT 1 FROM tbl_river_manual_sampling LIMIT 1');
    } on DatabaseException catch (e) {
      addLog("SQLite error: ${e.toString()}");
      addLog("Tip: Make sure 'tbl_river_manual_sampling' is created before pushing.");
      return PushRiverManualSample(rowsPushed: 0, imagesUploaded: 0, logs: _logs);
    }

    // 2) Count & fetch
    final result =
    await db.rawQuery('SELECT COUNT(*) as count FROM tbl_river_manual_sampling');
    final count = Sqflite.firstIntValue(result) ?? 0;

    if (count == 0) {
      addLog('No record in tbl_river_manual_sampling.');
      return PushRiverManualSample(rowsPushed: 0, imagesUploaded: 0, logs: _logs);
    }

    final records = await db.query('tbl_river_manual_sampling');
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'action': 'push-river-manual-sample', 'data': records}),
      );
    ...
    try {
      ...
      final parentFolderPath = path.join(baseDir.path, 'manual_sample');
    ...
    try {
      await db.delete('tbl_river_manual_sampling');
      addLog("Table 'tbl_river_manual_sampling' emptied.");
    } catch (e) {
      addLog("Failed to clear 'tbl_river_manual_sampling': $e");
    }
