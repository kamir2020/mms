...
import '../ftp-marine-in-situ-image.dart';
import '../ftp-marine-in-situ.dart';
...
class _SFormInSituSample extends State<SFormInSituSample> {
  ...
  void _scanBluetooth() {
    ...
    // Start the 30s timer once on the first reading
    if (_enableTimer == null) {
      _enableTimer = Timer(Duration(seconds: 30), () {
        if (mounted) {
          setState(() {
            _proceedEnabled = true;
          });
        }
      });
    }
  ...
  void dispose() {
    ...
    _tempData.dispose();
    _ecData.dispose();
    _sanilityData.dispose();
    _tdsData.dispose();
    _tssData.dispose();
    _turbidityData.dispose();
    _batteryData.dispose();
  ...
  void _sendDataServer() async {
    ...
    if (data['statusCode']==404) {
      ...
      // send data text (json) to FTP
      final payload = {
        "tech_name": firstSamplerName,
        "sampler_2ndname": secondSamplerName,
        "sample_date": dateController,"sample_time": timeController,"sampling_type":_type,
        "sample_state": _selectedStationName.text.toString(),
        "station_latitude": _getLatitude,"station_longitude": _getLongitude,
        "latitude": latitude!,"longitude": longitude!,
        "distance":_distanceDevice.toString(),
        "station_id":stationID,"timestamp":timestampResult1!
      };

      final status = await MarineManualInSituFTP.sendToFTP(payload: payload);
      print('upload file -->' + status);

      // send data image (.jpg) to FTP
      final imageList = [
        _image1,
        _image2,
        _image3,
      ...
      // If you have XFile? from image_picker, convert to File?
      File? toFile(var x) => x == null ? null : File(x.path);

      // Maintain order: optional1..optional8
      final ListImages = [
        toFile(_image1),
        toFile(_image2),
        toFile(_image3),
      ...
      final upload_status = await MarineInSituFtpMedia.zipAndUploadImages(
        images: ListImages,
        stationID: stationID,            // e.g. "6QBLG002"
        timestamp: timestampResult1!,    // e.g. "2025-08-15 20:25:13"
        remoteDir: '/marine/in_situ',            // auto-created if missing
        // host/user/pass/port already default to your server
      );

      print('upload images' + upload_status);
