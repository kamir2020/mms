...
class SFormMarineManualImageRequest extends StatefulWidget {
  _SFormMarineManualImageRequest createState() => _SFormMarineManualImageRequest();
}

class _SFormMarineManualImageRequest extends State<SFormMarineManualImageRequest> {
  ...
  Future<void> _loadLocation(String? stateID,String? categoryID) async {
    if (categoryID.toString()=='1') { // load station in-situ
      print('insitu');
    ...
    else if (categoryID.toString()=='2') { // load station tarball
      print('tarball');
      final data = await DBHelper.getLocationImageTarball(stateID);
      setState(() {
        _isLocation = true;
        location1 = data;
      });
  ...
  // ====== API ======
  Uri _buildUri() {
    const base = 'https://mmsv2.pstw.com.my/get-marine-manual-image-request.php';
              ...
              Expanded(
                child: DropdownSearch<localState>(
                  ...
                  onChanged: (localState? value) async {
                    ...
                    if (stateID != null) await _loadLocation(stateID,_selectedItemId.toString());
                  },
                ),
              ),
            ],
