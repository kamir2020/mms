...
class MarineTarballService {
    ...
    Future<Directory> Function()? getDocsDir,
      ...
      Uri.parse('https://mmsv2.pstw.com.my/api/marine/api-push-data.php'),
        uploadApiUrl = uploadApiUrl ??
            Uri.parse(
                'https://mmsv2.pstw.com.my/upload-image-local.php?image=marine_tarball'),
        _openDb = openDb,
  ...
  Future<PushTarballResult> pushDataTarballToMySQL({Logger? log}) async {
    final _logs = <String>[];
    ...
    try {
      ...
      await db.rawQuery('SELECT 1 FROM tbl_marine_tarball LIMIT 1');
    } on DatabaseException catch (e) {
      addLog("SQLite error: ${e.toString()}");
      addLog("Tip: Make sure 'tbl_marine_tarball' is created before pushing.");
      return PushTarballResult(rowsPushed: 0, imagesUploaded: 0, logs: _logs);
    }

    // 2) Count & fetch
    final result =
    await db.rawQuery('SELECT COUNT(*) as count FROM tbl_marine_tarball');
    final count = Sqflite.firstIntValue(result) ?? 0;

    if (count == 0) {
      addLog('No record in tbl_marine_tarball.');
      return PushTarballResult(rowsPushed: 0, imagesUploaded: 0, logs: _logs);
    }

    final records = await db.query('tbl_marine_tarball');
    ...
    try {
      ...
      final parentFolderPath = path.join(baseDir.path, 'tarball');
    ...
    try {
      await db.delete('tbl_marine_tarball');
      addLog("Table 'tbl_marine_tarball' emptied.");
    } catch (e) {
      addLog("Failed to clear 'tbl_marine_tarball': $e");
    }
