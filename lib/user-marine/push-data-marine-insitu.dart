...
class MarineInSituService {
    ...
    Future<Directory> Function()? getDocsDir,
      ...
      Uri.parse('https://mmsv2.pstw.com.my/api/marine/api-push-data.php'),
        uploadApiUrl = uploadApiUrl ??
            Uri.parse(
                'https://mmsv2.pstw.com.my/upload-image-local.php?image=marine_insitu'),
        _openDb = openDb,
  ...
  Future<PushInSituResult> pushDataInSituToMySQL({Logger? log}) async {
    final _logs = <String>[];
    ...
    try {
      ...
      await db.rawQuery('SELECT 1 FROM tbl_marine_insitu_sampling LIMIT 1');
    } on DatabaseException catch (e) {
      addLog("SQLite error: ${e.toString()}");
      addLog("Tip: Make sure 'tbl_marine_insitu_sampling' is created before pushing.");
      return PushInSituResult(rowsPushed: 0, imagesUploaded: 0, logs: _logs);
    }

    // 2) Count & fetch
    final result =
    await db.rawQuery('SELECT COUNT(*) as count FROM tbl_marine_insitu_sampling');
    final count = Sqflite.firstIntValue(result) ?? 0;

    if (count == 0) {
      addLog('No record in tbl_marine_insitu_sampling.');
      return PushInSituResult(rowsPushed: 0, imagesUploaded: 0, logs: _logs);
    }

    final records = await db.query('tbl_marine_insitu_sampling');
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'action': 'push-marine-insitu', 'data': records}),
      );
    ...
    try {
      ...
      final parentFolderPath = path.join(baseDir.path, 'manual_in-situ');
    ...
    try {
      await db.delete('tbl_marine_insitu_sampling');
      addLog("Table 'tbl_marine_insitu_sampling' emptied.");
    } catch (e) {
      addLog("Failed to clear 'tbl_marine_insitu_sampling': $e");
    }
